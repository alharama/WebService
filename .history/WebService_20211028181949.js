    ...
    connection.query("SELECT song FROM music", function (err, result, fields) {
        ...
        if (err) {
            ...
            if(Object.keys(result).length == 0) {
                ...
                Object.keys(result).forEach(function(key) {
                    console.log("key is " + result[key].song);
                    console.log("cur_song is " + given_song);
                    console.log(typeof result[key].song);
                    var curRow = JSON.stringify(result[key].song);
                    if(JSON.stringify(result[key].song) === given_song) {
